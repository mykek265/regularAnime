...
s2dpl::Primitiv::Primitiv(b2World& sworld, sf::RenderWindow& swindow, const sf::Vector2f& spos, float density, float friction, float elasticity, int type, uint16 rim, uint16 ricol)
	:pos(spos),
	angle(0.f),
	im(rim),
	iColWth(ricol)
...
s2dpl::Sensor* s2dpl::Primitiv::addSensor(sf::Vector2f spos, sf::Vector2f size)
{
	...
	jdef.bodyB = a->physics;//сохран€етс€ в векторе sensorid, так что тут нет криминал
...
s2dpl::Primitiv::~Primitiv()
{
	...
	circle.clear();
	triangle.clear();
	// ”даление составных атрибутов
	std::for_each(jj.begin(), jj.end(), [](b2Joint*j){ if(j) world->DestroyJoint(j); });
	jj.clear();
	std::for_each(ptrSensor.begin(), ptrSensor.end(), [](s2dpl::Sensor* s){ delete s; });
	ptrSensor.clear();
	bool del = false;
	// ”даление физической части
	if (physics)
	{
