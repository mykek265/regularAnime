...
namespace s2dpl
{
	...
	public:
		...
		Primitiv(b2World& sworld, sf::RenderWindow& swindow, const sf::Vector2f& pos, float density, float friction, float elasticity, int type, uint16 im, uint16 icol);
		...
		~Primitiv();
	private:
		Primitiv(const s2dpl::Primitiv& a);
		s2dpl::Primitiv& operator=(const s2dpl::Primitiv& a);
	};
}
